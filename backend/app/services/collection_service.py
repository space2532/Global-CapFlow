...
async def fetch_index_tickers() -> List[str]:
    ...
    def _sync_job() -> List[str]:
        ...
        for item in WIKI_INDEX_SOURCES:
            ...
            try:
                resp = session.get(url, timeout=10) # Using cached session
        ...
        for item in WIKI_INDEX_SOURCES:
            ...
            try:
                # pandas.read_html로 위키피디아 테이블 읽기
                tables = pd.read_html(url)
                
                # 각 테이블에서 티커 컬럼 찾기
                for table in tables:
                    # 티커 컬럼 이름 후보들
                    ticker_columns = ["Symbol", "Ticker", "Ticker symbol", "Symbols", "Code"]
                    
                    for col_name in ticker_columns:
                        if col_name in table.columns:
                            # 티커 추출 및 정제
                            tickers = table[col_name].dropna().astype(str)
                            for ticker in tickers:
                                # 공백 제거 및 대문자 변환
                                ticker = ticker.strip().upper()
                                # 특수 문자 제거 (예: "AAPL" 또는 "AAPL (Class A)" -> "AAPL")
                                ticker = ticker.split()[0] if ticker else ""
                                if ticker and len(ticker) <= 10:  # 합리적인 티커 길이
                                    all_tickers.add(ticker)
                            break  # 티커 컬럼을 찾으면 다음 테이블로
                
            except Exception:
                # 개별 지수 페이지 파싱 실패해도 나머지는 계속 시도
                continue
...
async def update_rankings_db(
    ...
    for item in top_100_list:
        ...
        if ticker in existing_companies:
            ...
            company.country = item.get("country") or company.country
