"""
Scheduler service wired for the ``app`` package namespace.
"""
...
def shutdown_scheduler() -> None:
    ...
    if scheduler is None:
        logger.info("Scheduler is not running. Nothing to shut down.")
        return
    ...
    try:
        ...
        logger.error(
            f"Error occurred while shutting down the scheduler: {type(e).__name__}: {e}",
            exc_info=True
        )
    finally:
        scheduler = None
